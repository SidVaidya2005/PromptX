...
- **Refresh after a mutation with `router.refresh()` from the client.** This line used to say `revalidatePath()` from the route handler, and that is wrong for this application: every route here reads cookies, so all of them are dynamic and none has a Full Route Cache entry for `revalidatePath()` to invalidate. What actually re-runs the sidebar's conversation query — which lives in the `(app)` layout, preserved across navigation — is `router.refresh()`. Proven at F07, relied on since by F08, F10, and F11. Reach for `revalidatePath()` only if a genuinely cacheable route is ever added. If `revalidateTag()` is ever needed it now requires a second `cacheLife` argument — the single-argument form is a type error.
