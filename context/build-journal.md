...
- **Still unverified: the active indicator, and not for want of a browser.** `/chat/[id]` has no route until F07, so every row currently links to the root 404 — which renders *outside* the `(app)` layout, with no sidebar at all. There is no way to put a row into its active state at F06. F07 creates that route and should confirm `aria-current="page"`, the `canvas-soft` fill, and the 2px `primary` left bar as part of its own pass.
