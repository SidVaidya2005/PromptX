...
- **`auth_leaked_password_protection` is still disabled**, carried from Phase 0 and nominally due here. Enabling it is a dashboard change and is safe: every test fixture authenticates with a `crypto.randomUUID()` password, and no real user has a password at all, since sign-in is Google-only. It guards a path that does not currently exist.
