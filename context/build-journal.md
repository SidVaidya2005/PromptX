...
### Feature 28 — Upload pipeline  *(2026-08-08)*

- Decision: **the send path landed here rather than in F29.** §28 lists the cap and the ready-only rule as server-side enforcement, so `chatSendSchema` gained a strict optional `attachmentIds`, validated above the "will reach a provider" line and linked below it. F29 adds the paperclip and the file parts on top of a rule already enforced. The alternative — defer, so no endpoint shape is built for a caller that does not exist yet — was the F24 argument and was rejected because it moves security-shaped work into a UI feature.
- Decision: **`'ready'` is measured, not claimed.** The confirm route reads each object's metadata back out of Storage and overwrites `size_bytes` and `mime_type` with what actually landed; a missing object is `409` and the row stays `pending`. Everything the draft carried was an assertion about bytes that never passed through this process. F17's ledger posture, applied to storage.
- Decision: **the reaper runs both passes**, not just §28's orphan rows. Pass 2 deletes objects with no row at all, which is the only cleanup that can reach files stranded by a conversation delete cascading to messages and then to attachment rows — a cascade is SQL, and SQL cannot touch the bucket. Candidates come from `orphaned_attachment_objects()`, a `security invoker` function granted to `service_role` only, because PostgREST does not expose the `storage` schema at all.
- Decision: **`link_attachments_to_message` is a database function**, for F19's reason — PostgREST cannot span a transaction and a message showing two of its four files is unrepairable — and it repeats the ready-and-unlinked conditions inside its own `where` so the check the route makes above the line cannot be raced by the write below it. `position` comes from the ordinality of the id array, so the composer's last arrangement is the only one that can win.
- Decision: **every Storage call lives in the data module.** The `.from()` lint rule catches `supabase.storage.from(…)` everywhere else, and the exempt folder is the right home anyway: one file holding the paths and the rows is what makes "deletes all three objects" checkable by reading.
- Decision: **the browser half shipped now, proven through a temporary harness** deleted before the final commit, as F01 did with its token proof page. Locking a server contract around an unproven `createImageBitmap` / `OffscreenCanvas` / EXIF path was the risk that bought the harness.
- Gotcha: **`verify_jwt` is not authorisation.** It checks only that a JWT was signed by this project, so any signed-in user could have invoked a function whose whole job is deleting things. It is off; the function proves the caller holds a service-role key by reading `shared_key_budget`, the table F02 left policy-less. A string comparison against an injected env var was rejected because `sb_secret_…` and the legacy `service_role` JWT are both valid and only one is a JWT.
- Gotcha: **F03's isolation fixture went red the moment the bucket limits existed** — `mime type application/octet-stream is not supported` on a text blob that had uploaded fine for five features, because `allowed_mime_types` was null the whole time. The fixture moved to a PNG; the limit stayed. That failure is the first evidence the limit does anything.
- Gotcha: **`playwright-cli eval` runs in an isolated world.** Two attempts to stub `OffscreenCanvas` from the console reported success and changed nothing in the page — `typeof` read `function` again on the next call, and the upload produced derivatives regardless. The no-derivative path was exercised with a PDF instead, which takes it through real code rather than through a stub.
- Gotcha: **`getBucket()` returns snake_case** (`file_size_limit`) while `info()` one method along returns camelCase (`contentType`). The first spelling of the limits test compiled and asserted `undefined` against a number. Read off the installed `Bucket` type, not guessed.
- Gotcha: `supabase/functions` is Deno and `tsconfig.json` included `**/*.ts`, so `pnpm typecheck` tried to compile `jsr:` imports and the `Deno` global. Excluded, with the reason written in the file.
- Verified: bucket row matches `MAX_ATTACHMENT_BYTES` and `ALLOWED_ATTACHMENT_MIME_TYPES`; an 11 MB upload and an SVG through **real signed URLs** both refused by Storage; a signed upload URL for another user's prefix refused by the F03 object policy.
- Verified: in a real browser, one image through the whole pipeline — derivatives, three uploads, confirm — with the objects downloaded back and measured: thumb 80×80 from every source, inline 1440×720 from a 2000×1000 original, 1200×800 and 60×40 left alone (never upscaled). A PDF produced one object with both derivative columns null.
- Verified: four attachments sent in a deliberately scrambled order came back at positions 0,1,2,3 in **that** order; the assistant answered; five attachments were refused by the schema cap; a pending id `400 attachment_not_ready`, an already-sent id `409`, a stranger's id `404`, duplicates `400`, and a confirm for an upload that never happened `409 upload_incomplete` — with **exactly one** conversation created across all of it.
- Verified: four mutations, each turning exactly the predicted test red and restored in the next call — dropping the ready check, the already-linked check, and the message-ownership guard from `link_attachments_to_message`, and nulling the bucket limits (3 red). The deployed definition was read back with `pg_get_functiondef`, per F16.
- Verified: **the ordering mutation**, which is the one no screen would have shown. Moving the attachment check below the provider line returned the identical `400 attachment_not_ready` and left a new "New chat" holding a prompt with no answer — plus a shared-key slot claimed by an early return that skipped the refund. Restored, the same request creates nothing.
- Verified: the reaper against seeded orphans — `{rowsDeleted:2, objectsDeleted:3}`, both orphan rows and their objects gone, a cascade-orphaned object with no row gone, a young draft untouched, and the four linked attachments and their ten objects untouched. A draft with no objects at all was reaped without `remove()` erroring. `reap_attachments_tick()` → pg_net → the function now returns a real body in `net._http_response` (200), after being green and inert since F02.
- Verified: 374 tests, `pnpm typecheck`, `pnpm lint`, and `pnpm build` all green, with both new routes in the route table and `/dev-upload` absent from it.

**Open after F28.**

- **A model still cannot see an attachment.** Storing and linking is this feature; the AI SDK file parts are F29 and capability gating is F30. Nothing in the UI can reach the state, because there is no composer control yet.
- **Remove, retry, and `status = 'failed'` are unbuilt**, by scope: §28 names failed rows only as something the send path must refuse. An abandoned upload is cleaned by the reaper meanwhile.
- **`createAttachmentReadUrls()` has no caller yet.** F29's message rendering is the caller; it was exercised by hand during this feature's browser pass.
