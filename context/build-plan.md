...
- **`isSharedKeyAvailable()` fails open**, because the authoritative cap is set at the provider and failing closed would trade a real outage for a few hypothetical cents. Feature 38 made that argument stronger rather than weaker: on the free tier the provider-side ceiling is a request quota costing nothing to breach, so failing closed would trade an outage for nothing at all
...
- **`/share/[slug]` reads through a genuinely anonymous client, and this is the load-bearing part.** F03's policies are `for select to anon`, and Postgres applies a policy only when the current role matches — Supabase sets `authenticated` for a signed-in JWT. So the cookie-bound client returns **nothing** for a signed-in visitor opening someone else's link, which is the ordinary case of a shared link being passed between two users of the product. `createAnonSupabaseClient()` sends no session, so every visitor reads through the same policies. **The obvious alternative is a data leak and was rejected outright:** widening the policies to `to anon, authenticated` would make `listConversations()` — which correctly carries no `user_id` filter, because RLS is the boundary — return *every shared conversation in the system* in *every* user's sidebar
- **A migration adds an anon read policy on `attachments`, scoped through the shared conversation.** §33 asks for placeholders and F03 wrote no anon policy on that table, so the share view cannot currently tell that a file was ever attached. The policy exposes metadata only — mime type, size, position — and the storage bucket's policies stay owner-scoped, so no object is readable. The share page must never call the signed-URL helper; that is the line the placeholder exists to keep
- **The slug comes from `crypto.getRandomValues` over a 64-character URL-safe alphabet**, not from `nanoid`, which is not a dependency and would be one for a single function. Web Crypto is a global in Node 26, so there is no import and nothing near the invariant naming `vault.ts` as the module that imports `node:crypto`. A 64-character alphabet divides 256 evenly, so masking the low six bits is unbiased with no rejection sampling — six bits × twelve characters is the ~71 bits `constants.ts` already documents
- **A new `ThreadHeader`, rendered by the chat surface rather than the shell.** §33 says "thread header" and there is none: `AppShell`'s is `desktop:hidden` chrome carrying the mobile sidebar and outline triggers, so extending it would put a share control on `/prompts`, `/search` and `/compare`, which have no conversation to share. The new one carries the title and an overflow menu, which §34 needs for export
- **Sharing an already-shared conversation keeps its slug.** `{shared: true}` is desired state, like pin and archive, so a second click must not break a link already pasted somewhere. Only revoke-then-share mints a new one
- **The page is `noindex`.** "Public to anyone who has it" is not "published", and an unguessable URL is still indexed once it is pasted anywhere a crawler reads
- **The share view renders its own read-only thread** rather than reusing `MessageBubble`, which is wired to edit, regenerate, outline anchors and highlight state. It reuses `MarkdownMessage` and `MarkdownErrorBoundary`, the split `CompareColumn` already made
...
- `render.yaml` Blueprint committed, defining a single Node Web Service:
  ...
  - `buildFilter.ignoredPaths` covering `context/**`, `docs/**`, `.claude/**`, and `*.md`, so a documentation commit does not rebuild and restart the service. On the free tier a restart hands the next visitor a ~60s cold start, which is a real cost for a change that alters no runtime behaviour. `docs/**` is on the list because the README screenshot lives there
  - Use `ignoredPaths`, never `paths` — an include list is fail-open in the wrong direction, silently dropping deploys for any path nobody remembered to list. A `buildFilter` fully replaces existing settings on sync, so the committed file is the whole truth rather than a patch on dashboard state. **Corrected at F38:** this line and `architecture.md` both said `ignored`/`included`, which are not Render's key names — and since a sync replaces rather than merges, the wrong spelling would have silently emptied the filter instead of failing
...
- Supabase production project, with the Auth redirect allowlist widened to the Render domain. **Amended at F38: the existing project `kplbxqujihxzdltrwxbw` is promoted in place rather than a new one migrated.** Replaying the 17 migrations into a fresh project would have been the first ever test of whether they replay from zero — a claim untested since F02 — but the cost lands elsewhere: both Vault secrets recreated by hand, the Edge Function redeployed, and a real chance of a half-built production database. The accepted price of promoting is that **`pnpm test` and `pnpm test:e2e` now write production**, which `constraints.md` records as a standing operational hazard rather than a passing note. The Google Cloud console entry is unchanged — it points at Supabase's `/auth/v1/callback`, never the app's, and the allowlist keeps `localhost` alongside the Render URL because one project now serves both
...
- ~~**A hard budget cap set on the Google Cloud billing account backing `SHARED_GEMINI_API_KEY`.**~~ **Superseded at F38: the shared key runs on Google's free tier, permanently, so there is no billing account and no bill to cap.** The requirement was right for the deployment it assumed and is met by a stronger mechanism than the one it named — the free tier's own hard quotas (20 requests/day, 5/minute) are enforced by Google, cannot be exceeded, and cost nothing when breached. The worst case is a 429, not a charge. What this *does* cost is that the ceiling is a request count rather than a dollar figure, which is why `NEXT_PUBLIC_SHARED_KEY_DAILY_MESSAGE_LIMIT` had to drop to 5 — see the entry above. Restore this requirement verbatim the day a billed key goes in
- `ENCRYPTION_KEY` stored only in Render's environment settings. **Amended at F38: the existing key is reused rather than generated fresh.** This line was written assuming production would be a separate Supabase project; F38 decided to promote the existing one instead, so development and production share a single `provider_keys` table. A fresh key would leave every stored row undecryptable while it still looked perfectly healthy in the table — `getDecryptedKey()` throwing `DecryptionError` on data that reads fine, which is the worst available shape for this bug. The instruction becomes correct again the moment production gets its own project
