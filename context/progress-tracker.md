...
**Last completed:** 15 Model picker — `ModelPicker` in the composer toolbar (one radio group across all four providers, keyed on a `provider:modelId` composite), `PATCH /api/conversations/[id]`, `updateConversationModel()`, and `isSharedModel()` / `isModelAvailable()` as the single definition the picker and `resolveModel()` both read. 162 tests, typecheck, lint, and a production build green. Verified in a browser: switching model mid-thread persisted, survived a reload, and left the earlier message recording `google` / `gemini-3.6-flash` while the next recorded `openrouter` / `anthropic/claude-opus-5`. All four PATCH refusals correct; 44px targets and no overflow at 360px on a coarse pointer. **One criterion is not closed** — a successful BYOK generation through the picker still 402s on OpenRouter credits, which is F14's uncapped-`maxOutputTokens` item rather than anything F15 introduced; the routing is proven by the failed row's `provider` and `model_id`
**Next:** 16 Shared-key quota — `src/server/quota.ts`, the atomic `reserve_shared_slot` upsert, and the composer's remaining-count indicator. The reservation goes into the marker already sitting in `resolveModel()`
