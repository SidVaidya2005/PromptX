...
**Phase:** Phase 1 — Core chat — all features built; the phase checkpoint is next
**Last completed:** 11 Delete conversation — an overflow menu in each sidebar row opens an `AlertDialog` naming the conversation; `DELETE /api/conversations/[id]` removes it and the cascade takes its messages. The row was restructured so a button could sit outside the link, and the timestamp and trigger share one grid cell so the title's width never moves. Verified in a browser: title identical at 150px in both states, focus landing on Cancel, Escape deleting nothing, the cascade confirmed in Postgres, the open conversation redirecting to `/chat` with Back not returning to it, another user's conversation 404ing with their row intact, and the whole flow again from the mobile drawer at 360px. Three bugs were found *in the browser* and none of them by the type checker — see the journal
**Next:** Phase checkpoint — verify Phase 1 is stable before starting Phase 2
...
1. **A hover-revealed control must default to visible and be *hidden* by `pointer-fine`, never revealed by `pointer-coarse`.** Written the other way round — `pointer-coarse:opacity-100` overriding an unscoped `group-hover:opacity-0` — the timestamp measured `opacity: 0` at 360px under `pointer: coarse`, the exact defect DESIGN.md forbids. Two causes compound: emulated and real touch still match `:hover` for the last-tapped element, and between two single-class rules the later one in the stylesheet wins, which is not something the class list shows you. Defaulting to visible means a variant that fails to match leaves a control reachable instead of stranded. Every `hover:`/`pointer-coarse:` pair already in the codebase is worth re-reading against this at F37. (F11)
