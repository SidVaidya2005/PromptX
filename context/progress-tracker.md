...
**Last completed:** 18 Message outline rail — 211 tests (12 new), typecheck, lint and a production build green. The rail turned out to be an architecture problem rather than a list-rendering one: it is slotted into `AppShell` by the layout while the messages live in `Chat` inside `children`, so the two are siblings and no prop reaches between them. `AppShell` now holds the outline and `Chat` publishes into it, which is what makes "no extra query" literally true — the entries are the same `useChat` array the thread renders. Three things were measured rather than assumed. The active-entry rule was **rewritten before shipping**: "last anchor to cross the line" is correct scrolling down and wrong scrolling up, so it records where each anchor *is* instead — verified holding one prompt through a long answer and walking back 3→2→1→0 on the way up. The entries memo was measured at **1 publish per streamed response against 14 without it**. Removing the publish effect's cleanup reproduced the stale rail on `/settings` exactly. Two constraint questions were settled by measurement, not reading: `OutlineRail` becoming a Client Component narrows F05's server-node rule rather than breaking it, and message anchors may carry ids because the thread renders once where a column renders twice — confirmed with the sheet open at 800px, two rail lists in the DOM and zero duplicate ids
**Next:** 19 Edit and resend — an inline textarea on a user message, `updateMessageAndTruncate()` deleting everything after it in one transaction, and a fresh completion from the truncated history. The rail rebuilds itself, which F18's content-keyed memo was already written to handle
