...
**Last completed:** 26 Full-text search backend — `search_messages` and `search_has_terms`, over a `search_vector` and GIN index that had sat unqueried since F02. Two facts were measured before any code: `ts_headline` does not escape the document it summarises, so snippets use control-character sentinels rather than `<mark>` and are plain text by construction; and a stopword-only query parses to an empty `tsquery`, so "no searchable terms" is a distinct answer from "no matches". §26's sub-500ms wall-clock assertion was replaced with `EXPLAIN ANALYZE` — 3.4–49.6ms over 5,001 rows, and the GIN index is correctly *not* used at this size, so no index was added. 348 tests, typecheck, lint and a production build green; advisors show only the documented notices
**Previously completed:** 25 Insert a prompt into the composer, then 24 Prompt library — the composer picker over a module-cached `GET /api/prompts`, and CRUD at `/prompts`. Each turned up one Radix control-flow defect that only driving the page could find
**Next:** 27 Search interface — `/search` with the query in the URL, results grouped by conversation, ⌘K from anywhere, and the renderer that turns F26's sentinels into real `<mark>` elements. Closes Phase 4
