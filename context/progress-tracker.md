...
**Last completed:** 25 Insert a prompt into the composer — a toolbar button opening an anchored popover with a search field and a keyboard-driven listbox, inserting the chosen body at the caret. A new `GET /api/prompts` corrects F24's prediction that this feature would reuse the page's server read; a module-level cache survives `Chat`'s remount-per-conversation, measured at exactly one request across three opens and a conversation switch, and F24's mutation hook invalidates it. One real defect found by driving the page: Radix returns focus to the trigger *after* the composer's own focus call, so insertion left focus on the button — fixed through `onCloseAutoFocus`, conditionally, so Escape still returns focus where it belongs. 337 tests, typecheck, lint and a production build green; two mutation runs each turned red exactly the tests that claim them
**Previously completed:** 24 Prompt library — CRUD at `/prompts` over a table that had sat unwritten since F02, with no migration needed. A sidebar nav block makes the page reachable and gives F27's Search and F31's Compare a slot; filtering is client-side over one server read; tags normalise in the schema, where `.overwrite()` rather than `.transform()` is what lets the count cap see the *deduplicated* tags. F23's deferred "Save to library" shortcut landed as an inline reveal. One real defect found by driving the page: the dialog seeded its fields in `onOpenChange`, which Radix never calls for a parent-driven open, so "New prompt" opened pre-filled and saved a duplicate — fixed by mounting the dialog only while open. 320 tests, typecheck, lint and a production build green; three mutation runs each turned red exactly the test that claims them
**Next:** 26 Full-text search backend — the `search_messages` function, `messages.search_vector` and its GIN index having existed unused since F02, the same shape F24 found the `prompts` table in
