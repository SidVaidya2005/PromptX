...
**Last completed:** 16 Shared-key quota — a migration adding `reserve_shared_slot` / `release_shared_slot` / `record_shared_tokens` (all `security invoker`), `src/server/quota.ts`, `src/server/data/shared-key-usage.ts`, `willUseSharedKey()`, the composer's quota meter and exhausted lock, and today's usage on `/settings/keys`. 180 tests, typecheck, lint, and a production build green. Verified end to end: the meter dropped on a real send, stopping a stream refunded the slot, a `POST /api/chat` at zero returned `429 quota_exceeded` and wrote nothing, and the exhausted composer unlocked on switching to a BYOK model. **One gap is on the record** — the concurrency test `build-plan.md` asks for cannot be built against the hosted project, because requests serialise; a deliberately racy implementation kept the suite green, which is how it was found
**Next:** 17 Global circuit breaker — the second, independent axis: `shared_key_budget`, `estimated_usd` from a Flash rate card, `BudgetExhaustedError` → 503, and the first `createServiceRoleClient()` in the codebase. It also owns F10's unmeasured titling spend
