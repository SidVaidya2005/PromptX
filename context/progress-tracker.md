...
**Last completed:** 13 Key management — the `/settings` section (Keys + Account, sub-nav), `POST`/`DELETE /api/keys`, `src/server/keys.ts` (free models-endpoint probe per provider, `InvalidKeyError` vs `ProbeUnavailableError`, `getDecryptedKey()`), and `src/server/data/provider-keys.ts` as the only module that knows `bytea` is hex over PostgREST. 138 tests, typecheck, lint, and a production build green. The bytea conversion was proven by writing and reading back through the real database, then proven *necessary* by mutation: passing a `Buffer` to supabase-js stores `{"type":"Buffer",…}`, reports success, and only fails much later at decrypt. Verified in a browser at 1280px and at 360px with a coarse pointer — a bogus key was refused by the real OpenAI endpoint with no row written, and no sealed column appeared in the DOM or the server HTML
**Next:** 14 Provider registry and model catalog — `src/lib/models.ts`, and `resolveModel()` extended to all four providers. The first caller of `getDecryptedKey()`
