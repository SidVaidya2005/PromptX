...
**Last completed:** 20 Regenerate — 237 tests (11 new), typecheck, lint and a production build green. `regenerate()` slices the assistant message out of client state *before* calling the transport, so `messages[messages.length - 1]` is by then the user's prompt: without a `trigger` branch the server appends it again and the thread holds one question twice. Proven by mutation — removing the branch produced a duplicate prompt and left the old answer undeleted. The destructive write is one `DELETE` by primary key, below the provider line; a second mutation hoisting it above `resolveModel()` returned the *identical* 429 and took the thread from two rows to one. **The design changed mid-verification:** the first version named the row to replace with the SDK's `messageId`, which is a database uuid only when the message came from the server — a message that has just streamed carries an SDK id, so the second consecutive regeneration sent `a6i8GltgJ1K7KZtE` and 400'd. The id was dropped entirely; the route already reads which answer to replace from the thread. Also measured: the outline rail's entries kept identical element refs across a regeneration, the control is 44px and persistently visible under coarse-pointer emulation, and a spent allowance greys out the shared model while leaving every keyed provider live
**Next:** 21 Rename and pin — `renameConversation()` capped at `MAX_TITLE_LENGTH`, `togglePin()` setting or clearing `pinned_at`, and a manual rename permanently suppressing auto-titling for that conversation
