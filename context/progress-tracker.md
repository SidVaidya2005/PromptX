...
**Phase:** Phase 2 — Bring your own key — **closed at its checkpoint**
**Last completed:** Phase 2 checkpoint — 199 tests, typecheck, lint and a production build green. Both `pg_cron` jobs confirmed succeeding in `cron.job_run_details`, and the reconciliation sweep finally *proven* rather than assumed: four new tests, two of which had never existed, showing it releases a stale orphan, leaves a live reservation alone, refuses to raise a counter, and retires an assistant row abandoned mid-stream. Two mutations landed exactly where intended — dropping the staleness guard zeroed a live reservation, dropping the lowering-only clause billed a user for a generation the reserve path had missed. **A gap in F17 was found and closed here**: `build-plan.md` §17 asked for that reconciliation test and F17 did not build it. The phase diff also turned up six comments describing work that had since shipped, and one of the corrections tripped `key-exposure.test.ts` by naming `getDecryptedKey` in a route comment — the guard greps `src/app` and cannot tell a comment from a call, so the comment bent rather than the test. `build-journal.md` compacted: Phase 2's per-feature entries collapsed 102 lines into 27, still-binding decisions promoted under their topics, and a new **Secrets and keys** section added
**Next:** 18 Message outline rail — the first feature of Phase 3, deriving the rail from the already-loaded thread with no extra query, tracking scroll with a throttled `IntersectionObserver`, and hiding itself below three exchanges
