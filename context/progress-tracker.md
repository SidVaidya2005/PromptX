...
**Phase:** Phase 2 — Bring your own key — all six features complete, checkpoint outstanding
**Last completed:** 17 Global circuit breaker — a migration adding `is_shared_key_available()` and `record_shared_budget()` (both `security invoker`, executable only by `service_role`), the codebase's first and only `createServiceRoleClient()`, the Gemini 3.6 Flash rate card, `BudgetExhaustedError` → `503 budget_exhausted`, and the breaker threaded pre-emptively into both chat pages, `/settings/keys` and `/chat`'s empty state. 195 tests, typecheck, lint and a production build green. Three mutations each turned exactly their own test red: dropping the `period_month` term, accumulating dollars per call instead of deriving them, and moving the breaker below the reservation. Verified in a browser against a real tripped breaker — composer locked with the meter hidden, `POST /api/chat` returning 503 having written nothing at all (not even a `shared_key_usage` row), and a BYOK model unlocking the composer while the breaker stayed tripped. **F10's unmeasured titling spend is closed**, observed rather than argued: one real message left `message_count = 1` while the ledger held 462 tokens against the message's own 88
**Next:** Phase 2 checkpoint — run the verification commands, inspect the phase diff for F12–F17, confirm `cron.job_run_details` shows both jobs succeeding now that real reservations exist, update this file, and compact `build-journal.md` by promoting Phase 2's still-binding decisions into Standing Constraints
