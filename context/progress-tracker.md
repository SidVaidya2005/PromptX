...
**Last completed:** 09 Message rendering — assistant bodies render GFM markdown with raw HTML off; fenced code carries the `code-block` chrome, a language label and a copy button, highlighted by a lazily-loaded shiki singleton in the browser once the message settles; each response gains a `message-meta` row with its model and a whole-message copy that yields raw markdown. A per-message error boundary falls back to plain text and recovers on the next delta. Verified against real Gemini: highlighting first appeared 235 ms *after* the stream ended and never during it, only the two grammars used were fetched, `<img src=x onerror=…>` from the model stayed literal text with zero elements created, and at 360px the page never scrolls sideways
**Next:** 10 Auto-generated titles
