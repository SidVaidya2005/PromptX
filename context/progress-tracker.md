...
**Last completed:** 08 Streaming responses on the shared key — `/api/chat` now returns a UI message stream; `src/server/providers.ts` lands with its final signature and the shared-Gemini branch only, marked where F14 and F16 slot in; one `Chat` client component drives both chat pages with a stop button. `SHARED_MODEL_ID` moved to `gemini-3.6-flash` — Google 404s `gemini-2.5-flash` for new users. Verified against real Gemini: incremental painting over 13.4s, stop persisting its partial answer, and every terminal state written by primary key
**Next:** 09 Message rendering
