...
**Phase:** Phase 0 — Foundation — **complete**, checkpoint passed
**Last completed:** 05 Application shell — the three-column frame (260/220, hairlines, no shadow), collapse persisted in a **cookie** so the server paints the collapsed state with no flash, a `Sheet` primitive driving the mobile drawer and outline sheet, the account menu with a POST sign-out, `error.tsx`/`loading.tsx`/root `not-found.tsx`, and `src/server/data/profiles.ts`. An exit animation was found to strand the panel on screen after Escape and was removed. The mobile layout — carried as *Not verified* by both F01 and F04 — is now verified at 360/768/1024/1440/1920
**Next:** 06 Conversation list (Phase 1 — Core chat)
...
5. **There is no local Supabase stack.** Migrations are authored as files under `supabase/migrations/` — still the source of truth — and delivered to the hosted project through the Supabase MCP. Consequences that bind everything downstream: no `supabase db reset`, so no migration is ever proven replayable from zero; types come from MCP `generate_typescript_types`, not `gen types --local`; and F03's RLS suite must run against the cloud instance. (F02)
6. **`cn()` must declare every custom token scale.** `tailwind-merge` knows only Tailwind's default class groups, so it dropped `text-button-md` when composed with `text-on-primary` and failed to dedupe `rounded-sm` against `rounded-pill`. `src/lib/utils.ts` uses `extendTailwindMerge`; a token added to `globals.css` and not to that list is silently unreliable. (F01, extended F05)
7. **Never use a named width utility.** The `DESIGN.md` spacing tokens shadow Tailwind's container scale — `max-w-md` is 10px, `max-w-xs` is 4px. Widths are numeric on the 4px grid (`max-w-112` = 448px), enforced by an ESLint rule. (F01)
8. **The `@theme` font tokens reference the `next/font` CSS variables**, not the family names, so the metric-matched fallback that prevents layout shift stays in the chain. (F01)
9. **shadcn primitives are retuned in place, with no alias layer.** One vocabulary; a freshly added component looks wrong until retuned, which is the point. (F01)
10. **The two quota limits are `NEXT_PUBLIC_*`.** Next inlines only that prefix into client bundles, and the composer displays the cap — unprefixed, the UI and the server would disagree silently. (F01)
