...
**Last completed:** 12 The encryption vault — `src/server/vault.ts` (AES-256-GCM, fresh 12-byte IV per encryption, mandatory auth tag, `lastFour()`, `DecryptionError`, no logging) and a 10-case suite; 105 tests, typecheck and lint green. Two contradictory invariants in `architecture.md` were resolved in favour of `env.ts` owning every secret read, and its vault snippet — which re-validated a length `env.ts` had already proven at boot — was corrected in place along with `library-docs.md`'s test guidance, which taught a `vi.stubEnv` pattern that cannot reach a module-load-time parse. The suite was proven able to fail by two separate mutations, which surfaced a real flaw in one test that had been passing by swallowing its own assertion
**Next:** 13 Key management — `/api/keys`, `getDecryptedKey()`, and the `/settings/keys` UI. The first consumer of the vault
