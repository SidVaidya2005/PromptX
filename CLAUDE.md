...
- **Commit subjects are `<phase>.<feature>.<seq> - <what changed>`**, where `phase` is the phase from `build-plan.md`, `feature` is the two-digit feature number, and `seq` counts from 1 within that feature. Feature 14 sits in Phase 2, so it is `2.14.1`, `2.14.2`. The last commit of a feature is conventionally the one that records it in `context/` ("Record feature NN…"). Read the number off `build-plan.md` rather than incrementing the previous commit's: features 12 and 13 opened Phase 2 but were committed as `1.12.x` / `1.13.x`, because the phase digit was carried forward from feature 11 instead of looked up. Those four are already published and were left alone; the rule is written here so the next phase boundary does not repeat it.
