...
- **Number every commit subject `<phase>.<feature>.<n>`**, followed by a plain lowercase imperative summary — `3.18.73 derive the outline rail from the loaded thread`. `<phase>` and `<feature>` are the current phase and feature numbers from `build-plan.md`, feature zero-padded to two digits. `<n>` is a **global counter that never resets across phases or features**: read the most recent commit (`git log -1 --format=%s`) and add one. Pad `<n>` to two digits and let it grow past `99` naturally. Read `<phase>` off `build-plan.md` rather than carrying it forward: features 12 and 13 opened Phase 2 but were committed as `1.12.x` / `1.13.x`, because the phase digit was inherited from feature 11 instead of looked up.
- **The global counter starts at 73**, continuing the 72 numbered commits that came before it. Those 72 used a per-feature `<n>` that reset each feature and a ` - Capitalized` summary; they are published and were left alone. The last of them is `2.17.10`, so the next commit is `<phase>.<feature>.73`.
