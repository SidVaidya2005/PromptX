...
export function ConversationRow({
  ...
  return (
    <li
      className={cn(
        'group relative flex items-center gap-sm rounded-sm pr-xs text-body-sm',
        // The 44px floor keys off pointer type, never off a breakpoint.
        'pointer-coarse:min-h-11',
        // The fill lives here rather than on the link, so hovering the trigger
        // still lights the row — and so the row does not go dark while its own
        // menu is open and the pointer has moved away to it.
        isActive
    ...
    >
      ...
      {/* The ::after overlay is what keeps the whole row clickable now that the
          link no longer wraps it. Without it the target shrinks to the width of
          the title text, and the empty space beside a short title stops working
          — which it did before this feature. */}
      <Link
        href={`/chat/${id}`}
        aria-current={isActive ? 'page' : undefined}
        className="min-w-0 flex-1 truncate py-sm pl-md after:absolute after:inset-0"
      >
        {title}
      </Link>
